...
use App\Modules\Remembrall\Command\ScheduleTodaysRemindersCommand;
...
class ScheduleTodaysRemindersCommandTest extends KernelTestCase
{
    use FixturesTrait;
    ...
    public function it_dispatches_schedule_pre_reminder_events_correctly(): void
    {
        ...
        $schedulePreRemindersCommand = new ScheduleTodaysRemindersCommand($reminderRepository, $dispatcher);
        $application->add($schedulePreRemindersCommand);

        $command = $application->find('remembrall:schedule-reminders');
        $commandTester = new CommandTester($command);

        $dispatcher->expects(self::once())->method('dispatch');
        $commandTester->execute([]);
        $output = $commandTester->getDisplay();
        self::assertStringContainsString('[OK] Zlecono zaplanowanie wysyłki 1 przypomnień', $output);
    }
}